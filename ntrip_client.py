...
#python ntrip_client.py -u hkast -p hkast941 -m 1000  -r 100 -v -D 9999  59.152.234.19 2103 HKST -f log/test.log -t 22.4264184 -g 114.2033127
...
logging.basicConfig(level=logging.DEBUG, 
                    ...
                    datefmt='%Y-%m-%d %H:%M:%S',
                    handlers=[logging.FileHandler('log/debug.log', 'w', 'utf-8'), ])
...
#        mountPointString = "GET %s HTTP/1.1\r\nUser-Agent: %s\r\n" % (self.mountpoint, useragent)
    ...
    def getGGABytes(self):
        ...
        if self.verbose:
            print("$%s*%s\r\n" % (ggaString, checksum))
            #logging.info(ggaString, checksum)
        ...
        try:
            while reconnectTry <= maxReconnect:
                ...
                if self.verbose:
                    ...
                    logging.info('Connection {0} of {1}\n'.format(reconnectTry,maxReconnect))
...
#                        print(casterResponse)
                    ...
                    while data:
                        try:
                            ...
                            rtr=RTCMReader(self.socket)
                            for(raw_data, parsed_data) in rtr.iterate(): 
                                print(parsed_data, '\n')
                            
                            #print("Recv retcm stream len: "+str(len(decode_data))+" bytes\n") 
                            #self.out.write('Recv rtcm stream len:{}\n'.format(len(decode_data))) 
                            if self.UDP_socket:
                            ...
                            if self.maxConnectTime :
                                if datetime.datetime.now() > connectTime+EndConnect:
                                    if self.verbose:
                                        ...
                                        logging.error("Connection Time exceeded\n")
                                    sys.exit(0)
#                            self.socket.sendall(self.getGGAString())
                    ...
                    if reconnectTry < maxReconnect:
                        ...
                        logging.error( "%s No Connection to NtripCaster.  Trying again in %i seconds\n" % (datetime.datetime.now(), sleepTime))
                    ...
                    self.socket=None
                    if self.verbose:
                        logging.error("Error indicator: ", error_indicator)
...
if __name__ == '__main__':
    ...
    parser.add_option("-t", "--latitude", type="float", dest="lat", default=22.30, help="Your latitude.  Default: %default")
    parser.add_option("-g", "--longitude", type="float", dest="lon", default=114.18, help="Your longitude.  Default: %default")
    parser.add_option("-e", "--height", type="float", dest="height", default=20, help="Your ellipsoid height.  Default: %default")
    ...
    if options.org:
        if len(args) != 1:
            logging.error("Incorrect number of arguments for IBSS. You do not need to provide the server and port\n")
            parser.print_help()
            sys.exit(1)
        ...
        if len(args) != 3:
            logging.error("Incorrect number of arguments for NTRIP\n")
            parser.print_help()
            sys.exit(1)
    ...
    if maxConnectTime < 0:
        ...
        logging.error("Max connection Time must be >= 0\n")
        sys.exit(1)

    if options.verbose or options.tell:
        logging.info("Server: " + ntripArgs['caster'])
        logging.info("Port: " + str(ntripArgs['port']))
        logging.info("User: " + ntripArgs['user'])
        logging.info("mountpoint: " +ntripArgs['mountpoint'])
        logging.info("Reconnects: " + str(maxReconnect))
        logging.info("Max Connect Time: " + str (maxConnectTime))
        if ntripArgs['V2']:
            logging.info("NTRIP: V2")
        else:
            logging.info("NTRIP: V1")
        if ntripArgs["ssl"]:
            logging.info("SSL Connection")
        else:
            logging.info("Uncrypted Connection")
        #print("")
